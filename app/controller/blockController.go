...
func (blockController *BlockController) BlockUser(c *gin.Context) {
	...
	if !exists {
		respondJSON(c, http.StatusUnauthorized, "你还没有登录喵，你要拉黑全世界吗？", nil)
		return
	}
...
func (blockController *BlockController) UnblockUser(c *gin.Context) {
	userIDValue, exists := c.Get("user_id")
	if !exists {
		respondJSON(c, http.StatusUnauthorized, "你还没有登录喵，服务器娘不知道你拉黑了谁", nil)
		return
	}
	userID := userIDValue.(uint)
...
func (blockController *BlockController) GetBlockedUsers(c *gin.Context) {
	...
	userIDValue, exists := c.Get("user_id")
	if !exists {
		respondJSON(c, http.StatusUnauthorized, "你还没有登录喵，服务器娘不知道你的小黑屋", nil)
		return
	}
	userID := userIDValue.(uint)
