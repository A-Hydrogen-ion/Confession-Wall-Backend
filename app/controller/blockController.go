...
func (blockController *BlockController) BlockUser(c *gin.Context) {
	...
	if !exists {
		ReturnErr(c, 401, "你还没有登录喵，你要拉黑全世界吗？")
		return
	}
	...
	if blockedIDStr == "" {
		ReturnErr(c, 400, "缺少参数 blocked_id 喵~")
		return
	}
	blockedID64, err := strconv.ParseUint(blockedIDStr, 10, 64)
	if err != nil {
		ReturnErr(c, 400, "blocked_id 参数格式错误喵~")
		return
	}
	...
	if err := service.BlockUser(database.DB, userID, blockedID); err != nil {
		ReturnErr(c, 500, "添加黑名单失败喵~")
		return
	}
	ReturnIMOK(c, "这个用户被你拉进小黑屋了喵~")
}
...
func (blockController *BlockController) UnblockUser(c *gin.Context) {
	userIDValue, exists := c.Get("user_id")
	if !exists {
		ReturnErr(c, 401, "你还没有登录喵，服务器娘不知道你拉黑了谁")
		return
	}
	userID := userIDValue.(uint)
	...
	if err := service.UnblockUser(database.DB, userID, blockedID); err != nil {
		ReturnErr(c, 500, "移除黑名单失败喵~")
		return
	}
	ReturnIMOK(c, "已成功将该用户移出黑名单了喵~")
}
...
func (blockController *BlockController) GetBlockedUsers(c *gin.Context) {
	...
	userIDValue, exists := c.Get("user_id")
	if !exists {
		ReturnErr(c, 401, "你还没有登录喵，服务器娘不知道你的小黑屋")
		return
	}
	userID := userIDValue.(uint)
